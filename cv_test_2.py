...
img = cv2.imread("ad_test_2.jpg")
img_bw = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
corners = []
bound = 3
attempts = 0
edges = cv2.Canny(img_bw, 150, 200, apertureSize = 3)
lines = cv2.HoughLinesP(edges, 1, numpy.pi/180, 150, minLineLength = 50, maxLineGap = 3)
#while len(corners) < 4:
if attempts == 1:
    bound = 300
corners = []
...
for i in range(len(lines[0])):
    ...
    ang_diff = abs(calc_ang(lines[0][i]) - max_line_ang)
    if (((calc_dist([pt[0],pt[1],max_line[0],max_line[1]]) < bound) or
         (calc_dist([pt[0],pt[1],max_line[2],max_line[3]]) < bound)) and
        (not pt == (-1,-1)) and abs(90 - ang_diff) < 5):
        cv2.circle(img, pt, 4, (0,255,0))
        corners += [pt]
        int_lines += [lines[0][i]]
        
if len(int_lines) >= 2:           
    greater = max(calc_dist(int_lines[x]) for x in range(len(int_lines)))
    print greater
    for line in lines[0]:
        if (not (line == max_line).all() and
            ...
            if abs(ang - max_line_ang) < 20:
                ...
                for int_line in int_lines:
                    pt = comp_intersect(line,int_line)
                    if (calc_dist([pt[0], pt[1], max_line[0], max_line[1]]) > greater and
                        calc_dist([pt[0], pt[1], max_line[2], max_line[3]]) > greater and
                        (not pt == (-1,-1))):
                        cv2.circle(img, pt, 4, (0,0,255))
                        int_count += 1
                if int_count == len(int_lines):
                    break
attempts += 1
